...
function Transactionrevenue({ index ,type, children }) {
  ...
  const searchdata = async () => {
    try {
      ...
      const apiresponse = await callapi(apiurls.revenue_report,{},"POST",body)
  ...
  const generaterepo = async () => {
    try {
      ...
      if (apiresponse) {
        success("Revenue Report Generate SuccessFully");
      } else {
          ...
          {tabledata != undefined && tabledata.length > 0 && (
            <MDBox p={3}>
              <DataTable
                table={{
                  columns: [
                    { Header: "Payment Sub Type", accessor: "PAYMENT_SUBTYPE", align: "left" },
                    ...
                    { Header: "Total Revenue", accessor: "Total", align: "center" },
                  ],
                  rows: tabledata,
                }}
                isSorted={true}
...
Transactionrevenue.defaultProps = {
  children: "",
};

// Typechecking props for the Header
Transactionrevenue.propTypes = {
