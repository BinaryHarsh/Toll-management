...
function Transaction_details() {
  ...
  const transaaction_list = useSelector((state) => state?.validate_list?.data?.transactions);
  ...
  // payment_type:'',plate_num:'',from_date:'',to_date:'',abnormality:'',tag_Id:'',vehicle_class:'',lane_type:'',transaction_id:'',lane:''
  const imgUrl = (time) => {
  ...
  const apicalling = async()=>{
    try{
      ...
      // const apiresponse = await callapi(apiurls.)
    }
    catch(error){
      console.log(error);
    }
  ...
  const counter = (val) => {
    ...
    if (transaaction_list != undefined) {
      ...
      if (index > -1) {
        ...
        navigate(`/transaction-details/${obj?.LANE_TRANS_ID ?? data?.id}`);
      }
    }
  };
  ...
  return (
    <DashboardLayout>
      ...
      <MDBox pt={4} pb={3}>
        ...
        <Grid item xs={12}>
            <MDBox>
              <MDButton variant="contained" onClick={()=>navigate(-1)} startIcon={"<"} color="info">
                Back
              </MDButton>
            </MDBox>
          </Grid>
