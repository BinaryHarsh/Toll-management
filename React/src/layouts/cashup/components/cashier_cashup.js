...
function Cashier_cashup({ index, type, children }) {
  const user = JSON.parse(localStorage.getItem("auth")).user ?? '';
  
  const [tabledata, setTabledata] = useState([]);
  const [qty, setQty] = useState(0);
  const [finaltotal,setFinaltotal] = useState(0)
  const [otherdetails,setOtherdetails] = useState({date:null,shift:''})
  const [collection, setCollection] = useState({upi: 0,card: 0,dd: 0,rtgs: 0,wallet: 0,cheque: 0});
  ...
  const resetForm=()=>{
    setQty(0);
    setFinaltotal(0);
    setOtherdetails({date:null});
    setCollection({upi: 0,card: 0,dd: 0,rtgs: 0,wallet: 0,cheque: 0});
    setApidata((apidata)=>apidata.map((val)=>{return {...val,["qty"]:0,["amount"]:0}}))
  }
  return (
        ...
        <Card>
          <MDBox px={4} py={3} >
            <List>
              <ListItem
                secondaryAction={
                  <MDdatepicker
                    ...
                    // label="Date :-"
                    onChange={(val,e)=>setOtherdetails((value)=>({...value,["date"]:val}))}
                  ...
                  <Box sx={{ minWidth: 150, width: "12.1rem" }}>
                    <FormControl fullWidth>
                      <Select
                        ...
                        name="cashier"
                        value={otherdetails.cashier}
                        onChange={(e)=>setOtherdetails((val)=>({...val,[e.target.name]:e.target.value}))}
                      >
              ...
              >
                <ListItemText
                  secondary={<MDTypography sx={{ fontSize: "1rem" }}>Cashier</MDTypography>}
                />
              </ListItem>
              <Divider variant="middle" component="li" />
              <ListItem
                  ...
                  <Box sx={{ minWidth: 150, width: "12.1rem" }}>
                    <FormControl fullWidth>
                      ...
                      name="shift"
                        ...
                        onChange={(e)=>setOtherdetails((val)=>({...val,['shift']:e.target.value}))}
                        style={{ minHeight: "2.5rem" }}
                      >
      ...
      <Grid item xs={4}>
        <Card>
          <MDBox p={2} display="grid" gap='1rem'>
            <MDButton color="info" fullWidth onClick={handleCalculate}>Calculate</MDButton>
            <MDButton color="success" type="submit" fullWidth>Submit</MDButton>
            <MDButton color="warning" onClick={resetForm}  fullWidth>Reset</MDButton>
          </MDBox>
        </Card>
      </Grid>
    </Grid>
...
Cashier_cashup.defaultProps = {
  children: "",
};

// Typechecking props for the Header
Cashier_cashup.propTypes = {
  children: PropTypes.node,
  index: PropTypes.number.isRequired,
  type: PropTypes.string.isRequired,
};

export default Cashier_cashup;
